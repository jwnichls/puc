...
public class EchoUIGenerator extends UIGenerator {

    //**************************
    // Member Variables
    //**************************

    protected LayoutVariables m_pVars;
    ...
    public EchoUIGenerator( MenuBar m ) {

	super( m );

	m_pWidgetRegistry = WidgetRegistryParser.parse( this.getClass(), 
							Globals.getRegistryFile() );
    }
	...
	/*
	 * There are at least two phases to generating the user
	 * interface of a remote control:
	 *
	 * 1: Generate organization from dependency information
	 * 2: Convert group tree to interface tree
	 * 3: Build concrete interface
	 *
	 */

	m_Panel.removeAllCIOs(); // remove any existing components
	m_pInterfaceRoot = new PanelNode();
	m_pVars = new LayoutVariables();
	m_pVars.m_bConstrainPanelsToScreenV = false;

	// Phase 1. Generate organization from dependency information

	generateOrganization( a.m_pRoot, a.m_vDependedObjects );

	// Phase 2. Convert group tree to interface tree

	generateHelper( a.m_pRoot, a, (PanelNode)m_pInterfaceRoot );

	// Phase 3. Build concrete interfaces from interface tree

	recoverRoot();
	m_pInterfaceRoot.setSize( Globals.getUIPaneSize().width,
				  m_pInterfaceRoot.getPreferredSize( m_pVars ).height );
	m_pInterfaceRoot.addComponents( m_Panel );
	m_pInterfaceRoot.doSizing( m_pVars );
	
	m_pInterfaceRoot.doLayout( m_Panel, m_pVars );
	
	// System.out.println( m_pInterfaceRoot.toString() );	
	
	this.setSize( m_Panel.getWidget().getSize() );
	...
	return m_pInterfaceRoot.getPreferredSize( m_pVars );
    }
